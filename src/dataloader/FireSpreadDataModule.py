...
class FireSpreadDataModule(LightningDataModule):
    ...
    def print_dataset_stats(self):
        ...
        # Get actual fire counts from each dataset
        def count_active_fires(dataset):
            if dataset is None:
                return 0
            return len(dataset.imgs_per_fire)  # Counts unique fire years
        
        def count_fire_events(dataset):
            if dataset is None:
                return 0
            total = 0
            for year in dataset.imgs_per_fire:
                total += len(dataset.imgs_per_fire[year])  # Counts fire events per year
            return total
        
        train_fires = count_fire_events(self.train_dataset)
        val_fires = count_fire_events(self.val_dataset)
        test_fires = count_fire_events(self.test_dataset)
        
        print(f"Train fires: {train_fires} - Samples: {len(self.train_dataset)}")
        print(f"Val fires: {val_fires} - Samples: {len(self.val_dataset)}")
        print(f"Test fires: {test_fires} - Samples: {len(self.test_dataset)}")
        
        # Print detailed fire info if available
        if hasattr(self.train_dataset, 'imgs_per_fire'):
            ...
            for dataset_name, dataset in [('Train', self.train_dataset), 
                ...
                if dataset:
                    ...
                    for year in dataset.imgs_per_fire:
                        print(f"  Year {year}: {len(dataset.imgs_per_fire[year])} fires")
    ...
    def setup(self, stage: str):
        ...
        print("\n=== DATASET INFO ===")
        print(f"Train years: {train_years} (count: {len(train_years)})")
        print(f"Val years: {val_years} (count: {len(val_years)})")
        print(f"Test years: {test_years} (count: {len(test_years)})")
