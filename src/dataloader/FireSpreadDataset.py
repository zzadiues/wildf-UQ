...
class FireSpreadDataset(Dataset):
    def __init__(self, data_dir: str, included_fire_years: List[int], n_leading_observations: int,
        ...
        print(f"[DEBUG] Initializing FireSpreadDataset with data_dir: {data_dir}")
        print(f"[DEBUG] HDF5 files found: {len(self.file_paths)}")
        print(f"[DEBUG] Sample indices available: {len(self.indices)}")

    def get_fire_counts(self):
        """Returns the number of unique fire events in this dataset"""
        count = 0
        for year in self.imgs_per_fire:
            count += len(self.imgs_per_fire[year])
        return count
